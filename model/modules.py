...
class BaseRGBModel(nn.Module, ABCModel):  # 修改为同时继承 nn.Module 和 ABCModel
    def get_optimizer(self, opt_args):
    ...
    def state_dict(self, *args, **kwargs):
        ...
        return super().state_dict(*args, **kwargs)  # 调用 nn.Module 的 state_dict

    def load(self, state_dict):
        if '_pred_fine._fc1._fc_out.weight' in state_dict:
            state_dict['_pred_fine._fc_out.weight'] = state_dict['_pred_fine._fc1._fc_out.weight']
            state_dict['_pred_fine._fc_out.bias'] = state_dict['_pred_fine._fc1._fc_out.bias']
            del state_dict['_pred_fine._fc1._fc_out.weight']
            del state_dict['_pred_fine._fc1._fc_out.bias']
            del state_dict['_pred_fine._fc2._fc_out.weight']
            del state_dict['_pred_fine._fc2._fc_out.bias']

        if isinstance(self, nn.DataParallel):
            self.module.load_state_dict(state_dict)
        else:
            super().load_state_dict(state_dict)  # 调用 nn.Module 的 load_state_dict
